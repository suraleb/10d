  ...
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      ...
      function addItem(title, value, preadded, locked, focusme) {
        if (!maxItems() || element.attr('disabled')) {
            return false;
        }

        if (options.onCorrectTitle.length) {
            title = options.onCorrectTitle(title);
        }

        if (options.onCorrectValue.length) {
            value = options.onCorrectValue(value);
        }
